$(document).ready(() => {
    document.body.style.backgroundImage = `url('static/img/fondo_${Math.round(Math.random() * (10 - 1) + 1)}.jpg')`;
    setInterval(() => {
        document.body.style.backgroundImage = `url('static/img/fondo_${Math.round(Math.random() * (10 - 1) + 1)}.jpg')`;
    }, 5000);
    const es_MX = {
    ...
    $('#tabla_mostrar_ips').DataTable({
        language: es_MX,
        responsive: true
    });
    $('#tabla_clases_ip').DataTable({
        language: es_MX,
        responsive: true
    });
    const entrada_fecha = document.getElementById("entrada_fecha");
    let fecha = new Date;
    entrada_fecha.value = (fecha.getFullYear() + `${(fecha.getMonth() + 1) < 10 && '-0'}` + (fecha.getMonth() + 1) + `${(fecha.getDay() + 1) < 10 && '-0'}` + (fecha.getDay() + 1));
});